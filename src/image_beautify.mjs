 ...
 *   - 微提亮 brightness 1.05
 *   - 增饱和 saturation 1.08（粉嫩感）
 *   - 微调整对比 (1.02, -3) 让肤色更柔
 *   - 极轻高斯模糊 0.4（接近不可感知，但抹掉硬边像 face smoothing）
 *
 * 故意不做激烈处理（避免假/网红/塑料感）。.env 设
 * IMAGE_BEAUTIFY_ENABLED=false 可关。
 *
...
export async function beautifyImage(buf, opts = {}) {
  ...
  try {
    const out = await sharp(buf)
      .modulate({
        brightness: opts.brightness ?? 1.05,
        saturation: opts.saturation ?? 1.08,
      })
      .linear(opts.contrast ?? 1.02, opts.contrastOffset ?? -3)
      .blur(opts.blur ?? 0.4)
