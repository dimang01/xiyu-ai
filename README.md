...
| 能力 | 简介 |
|---|---|
| **她会睡觉 (v1.10.0)** ⭐⭐ | 每个 companion 一份作息（入睡/起床时刻 + ±N min 抖动 + 学习状态）。睡眠时段你发的消息**完全静默**——不"正在输入"也不回，进 missed 队列，起床后用一条"看到你昨晚发了好多 \|\| 我刚醒"概括式回复。前 7 天观察用户作息（首条/末条消息时间），第 8 天自动按中位数固化（也可手动设）。dashboard 加 **📞 打电话叫醒她**：立刻解除睡眠，但她带起床气 annoyance+8/anger+5，第二次叫醒线性升级 |
