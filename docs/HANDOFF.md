...
| 2026-06-13 | **A/B 出图授权（单次·非常规·边界明确）**：维护者授权本次跑 8 张(OLD/NEW 各 4 seed)image 配额**仅限这一次 A/B 取证**——**后续任何烧图配额操作仍需单独点头**（默认只读授权不含烧配额的外部调用）。Claude 跑法=开发克隆 feat/avatar-realism-ab 分支，`DOTENV_CONFIG_PATH=/opt/.env` 注入生产 image provider + **`DB_PATH=/tmp` 覆盖防连生产库** + 落图 `/opt/xiyu-ai-new/public/avatars/ab_test`(nginx `^~ /avatars/` 直出)→图 URL 贴 PR #316。维护者倾向 NEW 但**出图对比为唯一定稿依据**，看图再拍 | 🔄 出图取证中 |
