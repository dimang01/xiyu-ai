...
| 2026-06-14 | **照片↔语境错配修拍板（接线第六案·方向 1+2 立刻上·走分支 PR）**：取证抓到原案(prod companion=3 06-13 15:53)——档案场景"逛同人摊淘画集"→随机采到 food 品类→caption 编"排队买拿铁"→机位 shot_mode=ACTIVITY_POV(无脸·已落库)→却载入裁脸 i2i 参考出脸。**根因(主)=权威机位信号 shotMode 在出图边界被丢弃**：`buildFinalImagePrompt`(photo_sender) 靠 `isSceneryScene(scenePrompt)` 文本嗅探 LLM 生成的 imagePrompt 重判"要不要脸"，而非上游已定死且落库的 shotMode(line 415 只传 identityPrompt/scenePrompt/refImagePath，shotMode 仅用于落库日志 line 452)；嗅探判错(食物 POV 当人像)→挂裁脸参考+identity→出脸。两种漂移(LLM 在 POV 里夹人物词 R2 / 嗅探漏判 R1)都收敛到同一句"shotMode 无确定性执行"。**接线第六案**：上游权威信号下游不消费、靠文本嗅探重判——解法同族(权威信号直穿执行点)，与通电冒烟测试并列。**修(本 PR·1+2)**：①shotMode 直穿 buildFinalImagePrompt 做确定性无脸闸——ACTIVITY_POV/SCENERY(=food/trophy/thought/activity_pov/sky 等 no-face 品类的机位)一律"do NOT show face"+不挂裁脸参考+不写 identityPrompt；isSceneryScene 退为 shotMode 缺失时兜底(user-request 老路)②sender 红线断言：no-face shotMode 绝不载入/使用裁脸参考；双向红验(喂夹"young woman/waist up"的食物 prompt·ACTIVITY_POV 下必须仍无脸·把 LLM 漂移也兜住)。**次因分开排(不在本 PR)**：品类↔current_scene 协同(proactive 照片主体由 current_scene/日程偏置而非纯随机)=current_scene(#326)天然延伸(记得在哪→拍在做的事)，标 v1.22.x·依赖 current_scene 稳定后做。**backlog 低优**：user 索图 decideShotMode 只认自拍/风景/作业三类，食物/物件落到拍脸。改完红验+回归(current_works/p0)全绿待合。**已落地 [PR #331](https://github.com/dimang01/xiyu-ai/pull/331)**：新增 `isNoFaceShot({shotMode,scenePrompt})` 单一真源(有 shotMode 信它·NO_FACE_SHOTMODES={ACTIVITY_POV,SCENERY}·缺失退 isSceneryScene 兜底)→ ①buildFinalImagePrompt 直收 shotMode 做确定性无脸闸 ②i2i 参考载入闸改用同源 isNoFaceShot(无脸机位绝不载入裁脸参考)；调用方(proactive+bot user 索图)皆已传 plan.shotMode。红验 photo_noface_gate_smoke 14/14(OLD isSceneryScene(leakyFoodPov)=false=会出脸 → NEW isNoFaceShot(ACTIVITY_POV)=true)+回归全绿(fix237 24/aspect 11/anchor 8/thought 13/planner 16/current_works 24+30/p0 127/lint 净)。CI 门禁 +1。**待维护者合(不碰部署)**。次因(品类↔current_scene 协同)标 v1.22.x 分开排；user 索图三类 backlog 低优 | ✅ PR #331 CI 绿·待合 |
| 2026-06-14 | **★ 产品转向留存·PR-0 真实留存看板（纯只读分析工具·走分支 PR·这是后续所有留存工作的北极星仪表+实验对照基线）**：背景="34/19 活跃天"是注册时 90 天模拟养成史撑的、真实 Day2≈1（取证已坐实：11 真实外部用户中只 4 个第二天回来过、全靠次日早安接回）。**严格边界：纯诊断、零改动生产行为**（不动 prompt/proactive/arc/emotion/persona/任何对话行为，纯算数出表）。**切分口径（写进脚本注释·可 SQL 复跑）**：真实活跃=`synthetic=0 AND role='user'`（backfill 养成史 synthetic=1；比 :00/:30 时间戳规律可靠——真实长聊有 2 条撞 :30）；proactive=assistant turn 在同 companion 同 created_at 无配对 user turn（回复对共享精确时间戳，standalone=主动/早安/场景照，91 条 vs 359 回复）；锚点 today=沪当天；剔除自有号 user1/2/3。**产出**=npm script `retention:board` 输出 ①per-user 表(Day0/Day1↩/Day2↩/Day7↩/真实活跃天/末次距今/**回来前最后一个 bot 触点**——看清"是什么把人接回来的") ②留存漏斗(N/Day1/Day2/Day7/≥2活跃日/近3天活跃) ③proactive 效果(60min 回复率/24h 零回复数/连≥2条没回 case) ④hard break 清单(复读/身份矛盾/场景矛盾后用户当场流失·定止血优先级)。**注意：companion_conversation_turns 每 companion 只留最新 100 turn(runHourlyCleanup)，高量用户真开场可能已裁——看板需对此诚实标注**。**跑出先停板贴维护者看数据再定 PR-1~4 顺序**（不预排死） | 🔄 PR-0 施工中 |
