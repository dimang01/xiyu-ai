...
| 2026-06-11 | 《用户协议》《隐私政策》正文起草：草稿放 drafts/ 等维护者逐句审定（附事实对照表），不自行部署；基调=诚实化（每句当前架构真实可兑现） | ✅ 草稿已出 |
| 2026-06-12 | 协议草稿六留白拍板：embedding(Gemini)列入第三方/站内公告改页面+邮箱通知/热线不写死号码(文书引用机制不引用常量)/邮箱 support@xiyuai.cc/中英平级互链/正文仅适用 xiyuai.cc+自托管须自行替换 | ✅ 已修订 |
